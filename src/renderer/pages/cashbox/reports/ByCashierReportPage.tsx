...
import type { ReportByCashierRow } from '@shared/types/transaction.types'

/** Onglet "Par caissier" — nouvel agrégat backend `getReportByCashier` (voir plan §3). */
export function ByCashierReportPage(): JSX.Element {
  const { filters } = useReportsStore()
  const [rows, setRows] = useState<ReportByCashierRow[] | null>(null)
  ...
  }, [filters.from, filters.to, filters.classId, filters.category])

  const columns = useMemo<ColumnDef<ReportByCashierRow>[]>(
    () => [
      { accessorKey: 'cashierName', header: 'Caissier' },
      {
    ...
    <div className="flex flex-col gap-4">
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Comparatif entrées / sorties par caissier</CardTitle>
        </CardHeader>
        <CardContent>
          <ComparativeBarChart
            data={rows.map((r) => ({
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Détail par caissier</CardTitle>
        </CardHeader>
        <CardContent>

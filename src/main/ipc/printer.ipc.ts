...
export function registerPrinterIpcHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.printer.updateConfig, async (_event, data: UpdatePrinterConfigDTO) => {
  ...
  ipcMain.handle(IPC_CHANNELS.printer.getStatus, async (): Promise<PrinterStatus> => {
    ...
    return {
      ...
      name: config.connectionType === 'network' ? (config.host ?? undefined) : (config.devicePath ?? undefined)
    }
  })
  ...
  ipcMain.handle(IPC_CHANNELS.printer.printReceipt, async (_event, receiptId: string): Promise<PrintResult> => {
    const config = printerConfigService.getPrinterConfig()
    if (!config.enabled) {
    ...
    try {
      ...
      const student = transaction.studentId ? studentService.findById(transaction.studentId) : null
      const className = transaction.studentId ? studentService.getCurrentClassName(transaction.studentId) : null
